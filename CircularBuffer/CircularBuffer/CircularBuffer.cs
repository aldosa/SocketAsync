...
namespace CircularBuffer
{
    public class CircularBuffer<T> : ICollection<T>, IEnumerable<T>, ICollection, IEnumerable
    {
        ...
        public int Head { get { return this.head;  } }
        ...
        /// <summary>
        /// Reads a portion of the buffer and copies it by ref to the target buffer.
        /// </summary>
        /// <param name="bufferToFill">The target buffer, populated by reference</param>
        /// <param name="offsetInFillBuffer">offset in the target buffer, the previous values are left as is</param>
        /// <param name="count">How many T items to copy to target</param>
        /// <returns>actual number of T items copied to target</returns>
        public int Get(T[] bufferToFill, int offsetInFillBuffer, int count)
        {
            int realCount = Math.Min(count, size);
            int dstIndex = offsetInFillBuffer;
            for (int i = 0; i < realCount; i++, head++, dstIndex++)
            {
                if (head == capacity)
                    head = 0;
                bufferToFill[dstIndex] = buffer[head];
            }
            size -= realCount;
            return realCount;
        }
